...
var CONTAINER_PATH = '/home/project';
var NPM_PATH = '/home/npm';
function compileDocker(config) {
    return function compile(sources) {
...
function createBuildCommand(config) {
    var configTag = config['docker-tag'];
    var tag = configTag ? ":" + configTag : ':stable';
    var allowedPaths = "\"" + CONTAINER_PATH + "," + NPM_PATH + "\"";
...
function getVolumes(config) {
    var hostPath = process.cwd();
    var hostNpmPath = path_1.join(hostPath, config.npmPath);
