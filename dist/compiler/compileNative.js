...
function createBuildCommand(config) {
    var command = 'solc';
    var params = '--standard-json';
    var customAllowedPaths = (config.allowedPaths || []).map(function (path) { return path_1.resolve(path); });
    var allowedPaths = [path_1.resolve(config.sourcesPath), path_1.resolve(config.npmPath)].concat(customAllowedPaths);
